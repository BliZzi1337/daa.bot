...
class USVEingabeMaske(discord.ui.Modal):
    def __init__(self, ziel: str):
        ...
        match ziel:
            ...
            case _:
                ...
                if ziel != "Akkus":
                    self.add_item(discord.ui.TextInput(label="Wirkungsgrad (%)", required=False, placeholder="leer = 100%"))
                if ziel == "Akkus":
    ...
    async def on_submit(self, interaction: Interaction):
        try:
            felder = {
                ...
                if isinstance(i, discord.ui.TextInput) and i.value 
                and "Wirkungsgrad" not in i.label and "Akku" not in i.label
            }
            ...
            einzelakku_text = next((i.value for i in self.children if "pro Akku" in i.label), "")
            einzelakku = float(einzelakku_text.replace(",", ".")) if einzelakku_text else None

            embed = discord.Embed(title=f"🔧 USV-Berechnung – {self.ziel}", color=discord.Color.orange())
            ...
            match self.ziel:
                ...
                case "Akkus":
                    if not einzelakku:
                        raise ValueError("⚠️ Kapazität pro Akku fehlt")
                    watt = felder["leistung"]
                    zeit = felder["zeit"] / 60
                    spannung = felder["spannung"]
                    energiebedarf = watt * zeit
            ...
            await interaction.response.send_message("❌ Fehler bei der Berechnung. Bitte prüfe die Eingaben.", ephemeral=True)

class USVAuswahlDropdown(discord.ui.Select):
    def __init__(self):
        optionen = [
            ...
            discord.SelectOption(label="Watt-VA", description="📐 W zu Scheinleistung (×1,55)"),
            discord.SelectOption(label="VA-Watt", description="📉 Scheinleistung zu W (×0,65)")
        ]
        super().__init__(
    ...
    async def callback(self, interaction: Interaction):
        ziel = ZIELTYPEN.get(self.values[0].strip(), "Unbekannt")
        await interaction.response.send_modal(USVEingabeMaske(ziel))

class USVRechnerView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(USVAuswahlDropdown())

class USVFormelCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @app_commands.command(name="usv", description="Interaktiver USV-Rechner mit Dropdown-Menü")
    async def usv(self, interaction: Interaction):
        embed = discord.Embed(
            title="🔧 USV-Rechner – Was möchtest du berechnen?",
            description=(
                "Wähle eine Zielgröße zur Berechnung im Dropdown-Menü.\n\n"
                "**Formeln:**\n"
